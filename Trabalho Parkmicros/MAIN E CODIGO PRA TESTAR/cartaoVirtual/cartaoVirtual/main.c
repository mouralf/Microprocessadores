...
float Buttons_validation(){
	...
	while(count != BOUNCE && contagem != 15){		//enquanto count não atingir 7 e espera sem toque no botão passar de determinado tempo com contagem
		delay_ms(500);								//delay para corrigir o debouncaa
...
int escreve_moedas(){
	...
	while(1){
		...
		if(contagem == 10){										//após certo tempo sem tocar em um botão, informa o dinheiro total colocado
			contagem = 0;									// reinicia contagem
		...
		if(val >= 4.50){
			contagem = 0;									// reinicia contagem
			LCD_control(0x80, CNFG);						// informa que o usuário não poderá inserir mais moedas, pois chegou ao limite
			enviaString("   ");
			enviaString("4,50");
